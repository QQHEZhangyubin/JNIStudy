...
void FffGcodeWriter::processInitialLayerTemperature(const SliceDataStorage& storage, const size_t start_extruder_nr, bool isFileHead)
{
    std::vector<bool> extruder_is_used = storage.getExtrudersUsed();
    Scene& scene = Application::getInstance().current_slice->scene;
    ...
    else if (gcode.getFlavor() != EGCodeFlavor::ULTIGCODE)
    {
        ...
        #if 0    //  delete  @2022-07-21 by CL
        //  add @2022-06-09 by CL  归零前温度
        bool tobeHoming = false;
        do{
            if(isFileHead) ;else{break;}
            if(scene.current_mesh_group->settings.get<bool>("material_print_temp_prepend")) ;else{break;}

            tobeHoming = true;  //  文件头的归零，改到这里
            ...
            for (unsigned extruder_nr = 0; extruder_nr < num_extruders; extruder_nr++)
            {
                if (extruder_is_used[extruder_nr])
                {
                    ...
                    Temperature before_homing_temperature = train.settings.get<Temperature>("before_homing_temperature");
                    ...
                    //  M104
                    if( std::abs(before_homing_temperature - extruder_temp) > 1){
                        gcode.writeTemperatureCommand(extruder_nr, before_homing_temperature);
                    }
                    else{
                        tobeHoming = false;
                        break;
                    ...
                    if (scene.current_mesh_group->settings.get<bool>("material_print_temp_wait"))
                    {
                        ...
                        gcode.writeTemperatureCommand(extruder_nr, before_homing_temperature, true);
                    }
                }
            }

            if(tobeHoming){
                gcode.writeCode("G28 ;Home");   //  add @2022-06-10 by CL  归零
            }
            tobeHoming = !tobeHoming;
        }while(false);
        #else
        bool tobeHoming = true;
        #endif
        ...
        if(tobeHoming){  //  add @2022-06-10 by CL  归零
...
void FffGcodeWriter::processStartingCode(const SliceDataStorage& storage, const size_t start_extruder_nr)
{
    ...
    if (Application::getInstance().communication->isSequential()) //If we must output the g-code sequentially, we must already place the g-code header here even if we don't know the exact time/material usages yet.
    {
        ...
        //  在文件头预留约 512B空间， add @2022-03-01 by CL
        //gcode.writeCode(prefix.c_str());
        //andrew 2022年11月16日16:24:16 新需求。 添加打印速度 300->400
    ...
    const Settings& mesh_group_settings = Application::getInstance().current_slice->scene.current_mesh_group->settings;
    if (mesh_group_settings.get<std::string>("param_print_mode") == "fast")
    {
    ...
    {
        //  isFileHead = true;  add @2022-06-10 by CL 文件头 归零前温度
    ...
    do{
        std::string start_gcode = mesh_group_settings.get<std::string>("machine_start_gcode");
        size_t found = start_gcode.find("G28");
        if(found != std::string::npos){
            start_gcode = start_gcode.insert(found, ";");
        }
        gcode.writeCode(start_gcode.c_str());
    }while(false);

    // add @2022-06-16 by CL XYZE轴 Acceleration Jerk
    ...
    if (mesh_group_settings.get<bool>("jerk_enabled"))
    {
        ...
        gcode.writeJerkXYZE('Z', mesh_group_settings.get<Velocity>("machine_max_jerk_z"));

        gcode.writeJerkXYZE('E', mesh_group_settings.get<Velocity>("machine_max_jerk_e"));
    }
    ...
    for (const size_t& extruder_nr : extruder_order)
    {
        ...
        if (layer_nr >= 0)
        {
            ...
            for (size_t mesh_idx : mesh_order)
            {
                ...
                gcode_layer.ptr_mesh_config = &mesh_config; //  add @2022-06-06 by CL
                gcode_layer.ref_data_storage.emplace_back(AkSliceDataStorageRef(storage, int(extruder_nr),  int(mesh_idx), int(layer_nr)) );
                ...
                gcode_layer.ptr_mesh_config = nullptr;
            }
        }
...
void FffGcodeWriter::addMeshLayerToGCode(const SliceDataStorage& storage, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, LayerPlan& gcode_layer) const
{
    ...
    if (mesh.isPrinted())
    {
        ...
        for (unsigned int part_idx = 0; part_idx < layer.parts.size(); part_idx++)
        {
            ...
            //andrew 2023年2月10日16:54:30 此处会出现part.insets[0].size() == 0的情况。导致闪退
...
bool FffGcodeWriter::processMultiLayerInfill(const SliceDataStorage& storage, LayerPlan& gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part) const
{
    ...
    for(unsigned int combine_idx = 1; combine_idx < part.infill_area_per_combine_per_density[0].size(); combine_idx++)
    {
        ...
        { // combine different density infill areas (for gradual infill)
            ...
            // 填充收缩距离  @2022-03-10 by CL //  Shrink == erode == (-)offset == (-)distance
            double  shrink_erode    = mesh.settings.get<double>("shrink_infill_shrink_distance");
            coord_t offset_distance = static_cast<coord_t>(shrink_erode * 1000);
            Polygons in_outline = part.infill_area_per_combine_per_density[density_idx][combine_idx];
            if(offset_distance)
                in_outline = in_outline.offset(-offset_distance);

            Infill infill_comp(infill_pattern, zig_zaggify_infill, connect_polygons, in_outline /* = part.infill_area_per_combine_per_density[density_idx][combine_idx]*/, /*outline_offset =*/ 0
...
//                if(1)   //  add @2022-08-04 by CL 拉桥区域
//                {
...
//                }
            ...
            //  @2022-05-22 by CL 悬垂皮肤   //2022/11/17 Modify by Binary top surface do not need overhang skin
            ...
            do{
                ...
                for(int hh=1; hh < part.outline.size(); ++hh){
                    ...
                    if(std::abs( hole.area() ) > MM2_2INT(3*3)){
                        hasAnyBigHole = true;
                        break;
                    }
                }
                ...
                gcode_layer.setHoleOverhangMask(hole_overhang_region);
            ...
            else
            {
                ...
                if(1) //  悬垂壁面积过滤 add @2022-05-22 by CL
                {
...
//                //andrew add begin
//                //5度一个梯度，根据overhang角度渐变速度
//                double stepAngle =5;
//                for(double angle = overhang_angle; angle > 30;angle = angle - stepAngle)
//                {
//                    const coord_t overhang_width = layer_height * std::tan(angle / (180 / M_PI));
//                    Polygons ext_overhang_region = part.outline.offset(-half_outer_wall_width).difference(outlines_below.offset(10 + overhang_width - half_outer_wall_width)).offset(10);
//                    gcode_layer.addExtOverhangMask(ext_overhang_region);
//                }

                //andrew add end

                /*
                //  由下面算法重新实现 @2022-05-13 by CL
                if(0){
                    //Polygons & temp_top      = overhang_region; //part.outline.offset(0);
                    //Polygons & temp_bottom_0 = outlines_below;
                    //Polygons   temp_bottom_1 = outlines_below.offset(10 + overhang_width - half_outer_wall_width);

                    Polygons temp_0 = overhang_region;
                    Polygons temp_1 = temp_0.offset(MM2INT(3.0)); temp_1.simplify(100, 50);
                    Polygons temp_2 = temp_0.offset(MM2INT(5.0)); temp_2.simplify(100, 50);
                    Polygons temp_3 = temp_0.offset(MM2INT(7.0)); temp_3.simplify(100, 50);
                    Polygons temp_4 = temp_0.offset(MM2INT(9.0)); temp_4.simplify(100, 50);

                    gcode_layer.setGradientVelocityMask(std::move(temp_0));
                    gcode_layer.setGradientVelocityMask(std::move(temp_1));
                    gcode_layer.setGradientVelocityMask(std::move(temp_2));
                    gcode_layer.setGradientVelocityMask(std::move(temp_3));
                    gcode_layer.setGradientVelocityMask(std::move(temp_4));
                }
                if(0 && overhang_region.size() > 0)    //  add @2022-05-13 by CL
                {
                    Polygons overhang_region_CL;
                    overhang_region_CL = overhang_region;
                    //std::list<Polygons> overhang_region_list{overhang_region.size()};

                    //log("$LayerNr \t Z \t length0 \t area0 \t L0/A0 \t  length1 \t area1 \t L0/A0 \t L0/L1 \t A0/A1 \n");
                    for(size_t ii=0; ii < overhang_region.size(); ++ii){
                        ConstPolygonRef polygon = overhang_region[ii];
                        Polygons temp0; temp0.add(polygon);
                        Polygons temp1 = temp0.approxConvexHull();
                        Polygons temp2 = temp0.offset(-half_outer_wall_width);
                        Polygons temp3 = temp2.offset(-half_outer_wall_width);

                        coord_t length0 = temp0.polygonLength();
                        double  area0   = temp0.area();

                        coord_t length1 = temp1.polygonLength();
                        double  area1   = temp1.area();

                        double  area2   = temp2.area();
                        double  area3   = temp3.area();

                        //if(length0 <= 0 || area0 <=0){continue;}

                        logCL("$CL$-LayerNr %d \t %d \t %f  \t %llu \t %f \t %f  \t %llu \t %f \t %f  \t %f \t %f \t %f \t %f \n"
                            , gcode_layer.getLayerNr(), ii, gcode_layer.z / 1000.0
                            , length0, area0, (area0 <= 0 ) ? 0 : (1.0 * length0*length0 / area0)
                            , length1, area1, (area1 <= 0 ) ? 0 : (1.0 * length1*length1 / area1)
                            , (length1 <= 0 ) ? 0 : (1.0 * length0 / length1), (area1 <= 0 ) ? 0 : (area0 / area1)
                            , area2, area3
                            );
                    }
                    gcode_layer.setOverhangMask(overhang_region_CL);
                }
                */
            }
        }
        ...
        else if (InsetOrderOptimizer::optimizingInsetsIsWorthwhile(mesh, part))
        {
            ...
            if(0x2 & mesh.settings.get<int>("optimize_single_part_z_seam"))
                ioo.z_seam_config.isPathOrPolygon = true;   // @2022-06-16 by CL  Z缝
        ...
        else
        {
            ...
            for (int inset_number = part.insets.size() - 1; inset_number > -1; inset_number--)
            {
                ...
                /*  delete @2022-07-13 by CL， 会使外墙形成单壁，单壁容易缺料
                //  add @2022-05-27 by CL  墙壁顺序
                int wallCount = part.insets.size();
                if(wallCount >= 3){
                    const bool optimize_wall_0_order = mesh.settings.get<bool>("optimize_wall_0_printing_order")
                            || (gcode_layer.getLayerNr() == 0 && mesh.settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::BRIM);

                    //logCL("$CL$ L, %s, %d, processed_inset_number = %d, wallCount=%d, wall_line_count=%d\n", __FUNCTION__, __LINE__ ,processed_inset_number, wallCount, mesh.settings.get<size_t>("wall_line_count"));
                    if(optimize_wall_0_order){
                        processed_inset_number = (processed_inset_number + wallCount - 1) % wallCount;
                        //logCL("$CL$ M %s, %d, processed_inset_number = %d\n", __FUNCTION__, __LINE__ ,processed_inset_number);
                    }
                }
                */
                ...
                if (processed_inset_number == 0)
                {
                    ...
                    if (part.insets[0].size() > 0 && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr)
                    {
                        ...
                        ZSeamConfig z_seam_config(mesh.settings.get<EZSeamType>("z_seam_type"), mesh.getZSeamHint(), mesh.settings.get<EZSeamCornerPrefType>("z_seam_corner"),
                            mesh.settings.get<coord_t>("z_seam_min_angle_diff") / 1000.f, mesh.settings.get<coord_t>("z_seam_max_angle") / 1000.f);
                        //  @2022-06-20 by CL  不在外表面生效
                        if(0x1 & mesh.settings.get<int>("optimize_single_part_z_seam"))
                            z_seam_config.isPathOrPolygon = true;   // @2022-06-16 by CL  Z缝
...
void FffGcodeWriter::processTopBottom(const SliceDataStorage& storage, LayerPlan& gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SkinPart& skin_part, Polygons& concentric_perimeter_gaps, bool& added_something) const
{
    ...
    //  @2022-05-21 by CL 修改悬垂皮肤的速率
    {
        do{
            bool    skin_overhang_enable         = mesh.settings.get<bool>("skin_overhang_enable");
            coord_t skin_overlap_overhang_mm     = mesh.settings.get<coord_t>("skin_overlap_overhang_mm");

            if(!skin_overhang_enable){break;}
            ...
            if(skin_config != &mesh_config.skin_config){break;}
            ...
            //2022/11/18 Binary 只有单层壁的情况，有时皮肤区域会超出模型  skin_overlap 不能太大
...
void FffGcodeWriter::finalize()
{
    ...
    else
    {
        ...
        {
            //  将统计信息重新写入文件头 @2022-03-01 by CL
            //andrew 2022年11月16日16:24:16 新需求。 添加打印速度 300->400
            gcode.writeCodeStored(prefix.c_str(), 0, 400);
        }
    }

    // delete @2022-06-16 by CL XYZE轴 Acceleration Jerk,移动到文件头了
    //if (mesh_group_settings.get<bool>("acceleration_enabled"))
    //{
    //    gcode.writePrintAcceleration(mesh_group_settings.get<Acceleration>("machine_acceleration"));
    //    gcode.writeTravelAcceleration(mesh_group_settings.get<Acceleration>("machine_acceleration"));
    ...
    gcode.writeComment(Application::getInstance().fffProcessor->getAllLocalSettingsString() + Application::getInstance().fffProcessor->getProfileString());
    */
}
