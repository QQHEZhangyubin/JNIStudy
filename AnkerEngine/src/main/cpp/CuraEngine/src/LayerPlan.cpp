...
//begin andrew 20220615 外墙在最小冷却是时不降速
, extrudeCoolMinLayerTimeSpeedFactor(1.0)
, extraTime(0.0)
, totalPrintTime(0)
{
}
...
void ExtruderPlan::setCoolMinLayerTimeSpeedFactor(const Ratio speed_factor)
{
    extrudeCoolMinLayerTimeSpeedFactor = speed_factor;
}

double ExtruderPlan::getCoolMinLayerTimeSpeedFactor()
{
    return extrudeCoolMinLayerTimeSpeedFactor;
}
...
LayerPlan::~LayerPlan()
{
    ...
    for(auto config : ptr_path_configs){
        delete config;
    }
    ptr_path_configs.clear();
}
...
void LayerPlan::addExtrusionMove_CL(Point p, const GCodePathConfig &config, SpaceFillType space_fill_type, const Ratio &flow, bool spiralize, Ratio speed_factor, double fan_speed)
{
    GCodePath* path = getLatestPathWithConfig(config, space_fill_type, flow, spiralize, speed_factor);
    path->points.push_back(p);
    path->setFanSpeed(fan_speed);
...
//                                     (overhang_mask.empty() || (!overhang_mask.inside(p0, true) && !overhang_mask.inside(p1, true))) ? speed_factor : overhang_speed_factor);
                    ...
                    do{
                        ...
                        if(gradient_velocity_mask[4].inside(p0, true) || gradient_velocity_mask[0].inside(p1, true)){
                            speed_factor_overhang = std::min((double)overhang_speed_factor * 5.0 , (double)overhang_speed_factor + 4.0 / 5 * (speed_factor-overhang_speed_factor));
                            logCL("$CL$ %s %d 4\n", __FUNCTION__, __LINE__);
                        }
                        else{break;}

                        if(gradient_velocity_mask[3].inside(p0, true) || gradient_velocity_mask[0].inside(p1, true)){
                            speed_factor_overhang = std::min((double)overhang_speed_factor * 4.0 , (double)overhang_speed_factor + 3.0 / 5 * (speed_factor-overhang_speed_factor));
                            logCL("$CL$ %s %d 3\n", __FUNCTION__, __LINE__);
                        }
                        else{break;}

                        if(gradient_velocity_mask[2].inside(p0, true) || gradient_velocity_mask[0].inside(p1, true)){
                            speed_factor_overhang = std::min((double)overhang_speed_factor * 3.0 , (double)overhang_speed_factor + 2.0 / 5 * (speed_factor-overhang_speed_factor));
                            logCL("$CL$ %s %d 2\n", __FUNCTION__, __LINE__);
                        }
                        else{break;}

                        if(gradient_velocity_mask[1].inside(p0, true) || gradient_velocity_mask[0].inside(p1, true)){
                            speed_factor_overhang = std::min((double)overhang_speed_factor * 2.0 , (double)overhang_speed_factor + 1.0 / 5 * (speed_factor-overhang_speed_factor));
                            logCL("$CL$ %s %d 1\n", __FUNCTION__, __LINE__);
                        }
                        else{break;}

                        if(gradient_velocity_mask[0].inside(p0, true) || gradient_velocity_mask[0].inside(p1, true)){
                            speed_factor_overhang = std::min((double)overhang_speed_factor * 1.0 , (double)overhang_speed_factor + 0.0 / 5 * (speed_factor-overhang_speed_factor));
                            logCL("$CL$ %s %d 0\n", __FUNCTION__, __LINE__);
                        }
                        else{break;}

                    }while(false);
                    logCL("$CL$ %s %d speed_factor_overhang: %f\n", __FUNCTION__, __LINE__ ,speed_factor_overhang);

                    addExtrusionMove(segment_end, non_bridge_config, SpaceFillType::Polygons, segment_flow, spiralize, speed_factor_overhang);
                }
            }
    ...
    {
        //  悬垂壁，线段降速。原始代码 @2022-06-22 by CL
        if(1){
            // no bridges required
    ...
    //andrew 2022年9月8日21:29:42 测试可以看出。这个else的代码会破坏polygon的封闭性
    else
    {
        //  拉桥 @2022-08-04 by CL
        //  std::string html_name = "T:\\_Share_for_VM\\debug_write.html";
        //  SVGHelper::appendPolygons(bridge_wall_mask, SVG::Color::BLACK, 3, html_name);
        //  SVGHelper::appendPolygons(bridge_wall_mask, SVG::Color::RED, 1, html_name);
        //  SVGHelper::writePolygons(bridge_wall_mask);

        //andrew 2022年9月8日21:44:09 这里缩小bridge_wall_mask，可以解决综合模型等破坏桥
...
//         return;
        ...
        // bridges may be required  可能需要桥梁
...
////            //  线穿过受支持区域和不受支持区域的边界，因此需要一个或多个桥来确定线的哪些段是桥
...
////                //  使用non_bridge_config挤出到下一个桥段的开始
////                addNonBridgeLine(b0);
...
////            //  如果还没有到达p1，则使用non_bridge_config行填补空白
////            addNonBridgeLine(p1);
//        }
...
//            //  线条的任何部分都不能露出水面，或者线条太短而不能作为桥线印刷
//            addNonBridgeLine(p1);
//        }
    }
}
//andrew 2022年7月26日20:47:39 求当前层上下n层的mask。 求悬垂区域，还要对应速度档位。
void LayerPlan::calcMasks(int layNr, const SliceMeshStorage& mesh, const GCodePathConfig& non_bridge_config_const, const SliceLayerPart& part, Polygons& out_bridge_wall_mask,Polygons& out_overhang_skin_mask, Polygons& out_overhang_hole_mask,Polygons& out_overhang_mask) const
{
    ...
    if (layNr > 0)
    {
        ...
        for (const SliceMeshStorage& m : storage.meshes)
        {
            if (m.isPrinted())
            {
                for (const SliceLayerPart& prevLayerPart : m.layers[layNr - 1].parts)
                {
        ...
        const coord_t layer_height = non_bridge_config_const.getLayerThickness();
        ...
        if (mesh_group_settings.get<bool>("support_enable"))
        {
            ...
            const int support_layer_nr = layNr - z_distance_top_layers;
        ...
        const int half_outer_wall_width = non_bridge_config_const.getLineWidth() / 2;
        ...
        if (mesh.settings.get<bool>("bridge_settings_enabled"))
        {
            ...
            // max_air_gap是墙线以下不支撑区域的最大允许宽度
            //如果不支撑的区域比max_air_gap宽，墙线将使用桥设置打印

            const coord_t max_air_gap = half_outer_wall_width;

            // subtract the outlines of the parts below this part to give the shapes of the unsupported regions and then shrink those shapes so that any that are narrower than two times max_air_gap will be removed
            // 减去该部分以下部分的轮廓，以给出不支撑支撑区域的形状，然后收缩这些形状，以便任何比max_air_gap的两倍更窄的将被删除

            Polygons compressed_air(part.outline.difference(outlines_below).offset(-max_air_gap));

            // now expand the air regions by the same amount as they were shrunk plus half the outer wall line width which is required because when the walls are being generated, the vertices do not fall on the part's outline
            // but, instead, are 1/2 a line width inset from the outline
            //现在扩大悬空区域相同的数量，因为他们收缩加上一半外墙线宽度，这是必需的，因为当墙壁被生成，顶点不落在部分的轮廓上
            //但是，相反，是插入线宽的1/2从轮廓

            out_bridge_wall_mask = compressed_air.offset(max_air_gap + half_outer_wall_width);
        }
        else
        {
            // clear to disable use of bridging settings
            out_bridge_wall_mask = Polygons();
        }

        if(mesh.settings.get<bool>("skin_overhang_enable")){
            const coord_t max_air_gap = half_outer_wall_width;
            Polygons compressed_air(part.outline.difference(outlines_below).offset(-max_air_gap));
            out_overhang_skin_mask =compressed_air.offset(max_air_gap + half_outer_wall_width);
        }
        else{
            out_overhang_skin_mask = Polygons();
        }
        do{
            if(part.outline.size() > 1)    ;else{break;}
            bool hasAnyBigHole = false; // isAllSmallHole
            for(int hh=1; hh < part.outline.size(); ++hh){
                ConstPolygonRef hole = part.outline[hh];
                if(std::abs( hole.area() ) > 9000000){
                    hasAnyBigHole = true;
                    break;
                }
            }
            if(hasAnyBigHole)    ;else{break;}

            const coord_t overhang_width = half_outer_wall_width / 2; //layer_height * std::tan(15 / (180 / M_PI));
            Polygons outline_LuJing = part.outline.offset(-half_outer_wall_width);
            Polygons outlines_below_XiaCengLuJing = outlines_below.offset(-half_outer_wall_width + overhang_width);
            Polygons overhang_region_XuanChuiLujing = outline_LuJing.difference(outlines_below_XiaCengLuJing);

            //double area = part.outline.area();
            Polygons hole_overhang_region;
            for(int hh=1; hh < part.outline.size(); ++hh){
                //Polygon  hole = part.outline[hh]; hole.reverse();
                Polygons holes; holes.add(Polygon(part.outline[hh]).reverse());
                Polygons holes2 = holes.offset(half_outer_wall_width*2).intersection(overhang_region_XuanChuiLujing);
                //logCL("===$CL$ %s %d, hh=%d, overhang_region_XuanChuiLujing = %f, holes=%f, holes2=%f\n", __FUNCTION__, __LINE__,hh, overhang_region_XuanChuiLujing.area(), holes.area(), holes2.area());
                if(!holes2.empty())
                    hole_overhang_region.add(holes2);
            }
            out_overhang_hole_mask = hole_overhang_region;
            //double area = hole_overhang_region.area();
            //if(area != 0)
                //logCL("$CL$ %s %d hole_overhang_region=%f\n", __FUNCTION__, __LINE__, hole_overhang_region.area());
        }while(false);
        ...
        if (overhang_angle >= 90)
        {
            ...
            out_overhang_mask = Polygons();
        }
        else
        {
            //  overhang_angle 悬垂壁角度 @2022-05-13 by CL
            //  悬垂遮罩设置为当前部件轮廓的区域减去被认为支持的区域 支持的区域由那些真正由模型支持的区域组成，或者下层的支持扩展为考虑悬垂角，悬垂角越大，认为支撑面积越大
            ...
            if(1) //  add @2022-05-22 by CL
            {
                const int64_t overhang_area_filter  = MM2_2INT(mesh.settings.get<double>("wall_overhang_area_filter"));
                Polygons overhang_region_temp;
                //double area = INT2MM2(sqrt( half_outer_wall_width * 4));
                for(int i=0; i < overhang_region.size(); ++i){
                    ConstPolygonRef polygon = overhang_region[i];
                    if(polygon.area() >= overhang_area_filter){
                        overhang_region_temp.add(polygon);
                    }
                }
                overhang_region = overhang_region_temp;
            }

            out_overhang_mask = overhang_region;
...
//  @2022-05-16 by CL 重点优化
void LayerPlan::addWall(ConstPolygonRef constWall, int start_idx, const SliceMeshStorage& mesh, const GCodePathConfig& non_bridge_config_const, const GCodePathConfig& bridge_config, WallOverlapComputation* wall_overlap_computation, coord_t wall_0_wipe_dist, float flow_ratio, bool always_retract)
{
    //  add @2022-06-07 by CL （non_bridge_config 这里用栈变量会卡死？？？）
    this->ptr_path_configs.emplace_back(new GCodePathConfig( non_bridge_config_const));
    GCodePathConfig &non_bridge_config = *this->ptr_path_configs.back();
    //GCodePathConfig &non_bridge_config = non_bridge_config_const;

    //andrew 2022年7月27日17:04:43 解除const
    Polygons tempWalls ;
    tempWalls.add(constWall);
    PolygonRef wall = tempWalls[0];
    double wall_area = wall.area(); //  add @2022-09-02 by CL
    //logCL("$CL$ %s %d addWall", __FUNCTION__, __LINE__);
    ...
    //  add  @2022-06-22 by CL
    const int64_t overhang_area_filter  = MM2_2INT(mesh.settings.get<double>("wall_overhang_area_filter"));
    //const bool overhang_extend_circle   = mesh.settings.get<bool>("wall_overhang_extend_circle");
    const double min_extrusion_mm       = mesh.settings.get<double>("wall_min_extrusion");

    Ratio wall_overhang_speed_factor = mesh.settings.get<Ratio>("wall_overhang_speed_factor");

    //最小悬垂长度
    const coord_t overhang_min_line_length = mesh.settings.get<coord_t>("overhang_min_line_length");
    ...
    //andrew 2022年9月9日16:02:59 将墙中跨悬垂/拉桥的线段拆分成多个线段。 这样便于减速
        ...
        for (int point_idx = 0; point_idx < singleWall.size(); point_idx++)
        {
            ...
            if (PolygonUtils::polygonCollidesWithLineSegment(slowMask, p0, p1))
            {
                ...
                while (line_polys.size() > 0)
                {
                    ...
                    {
                        //b0,b1 内缩一下在加入wall。 这样在拉桥/悬垂结束前开始加速。保持上岸后可以和其他速度一致。
                        auto dir = b1-b0;
    ...
    //andrew 计算singleWall上所有点到mask区域的距离
    ...
    {
        //在外部clear。这个map需要累计各种因素的影响
        //distanceToOverhangAreaMap.clear();

        std::unordered_map<int, double> DistanceCacheMap;
        DistanceCacheMap.clear();
        //默认10米，表示没有悬垂区域
        double distanceToOverhangStart = MM2INT(10000);
        std::vector<int> zeroIdx;

        //遍历整圈，记录每个点到悬垂区的距离。这里有可能多个悬垂区域，理论上需要记录每个点到每个悬垂区域的最短距离，但一个点只会有一个速度，所以只需要记录一个最短距离就可以了
        for (int point_idx = 0; point_idx < singleWall.size(); point_idx++)
        {
            if (targetMask.empty())
            {
            ...
            if (PolygonUtils::polygonCollidesWithLineSegment(targetMask, p0, p1))
            {
                ...
                {
                    //前面分割过线段了。 所以这里只承认在mask区域内的是悬垂，其他都不是
        ...
        //正反绕圈跑，得到每个点离悬垂点的距离
        std::unordered_map<int, double> DistanceToOverhangMapAntiClockwise;
        std::unordered_map<int, double> DistanceToOverhangMapClockwise;
        for (size_t i = 0; i < zeroIdx.size(); i++)
        {
            int zeroIndex = zeroIdx[i];
            //逆时针跑一圈计算每个点离0点的距离
            int curIndex = (zeroIndex + 1) % singleWall.size();
            do
            {
                int preIndex = (singleWall.size() + curIndex - 1) % singleWall.size();
                if (DistanceCacheMap[curIndex] != 0)
                {
                    //contains
                    if (DistanceToOverhangMapAntiClockwise.find(preIndex) != DistanceToOverhangMapAntiClockwise.end())
                    {
                        DistanceToOverhangMapAntiClockwise[curIndex] = DistanceToOverhangMapAntiClockwise[preIndex]  + DistanceCacheMap[curIndex];
                    }
                    else
                    {
                        DistanceToOverhangMapAntiClockwise[curIndex] = DistanceCacheMap[curIndex];
                    }
                }
                else
                {
                    DistanceToOverhangMapAntiClockwise[curIndex] = 0;
                }
                curIndex = (curIndex + 1) % singleWall.size();
            } while (curIndex != (zeroIndex + 1) % singleWall.size());

            //顺时针跑一圈计算每个点离0点的距离
            curIndex = (singleWall.size() + zeroIndex - 1) % singleWall.size();
            do
            {
                int preIndex = (curIndex + 1) % singleWall.size();
                if (DistanceCacheMap[curIndex] != 0)
                {
                    //!contains
                    if (DistanceToOverhangMapClockwise.find(preIndex) != DistanceToOverhangMapClockwise.end())
                    {
                        DistanceToOverhangMapClockwise[curIndex] = DistanceToOverhangMapClockwise[preIndex]  + DistanceCacheMap[curIndex];
                    }
                    else
                    {
                        DistanceToOverhangMapClockwise[curIndex] = DistanceCacheMap[curIndex];
                    }
                }
                else
                {
                    DistanceToOverhangMapClockwise[curIndex] = 0;
                }
                curIndex = (singleWall.size() + curIndex - 1) % singleWall.size();
            } while (curIndex != (singleWall.size() + zeroIndex - 1) % singleWall.size());
        }

        //取最小值
        for (int point_idx = 0; point_idx < singleWall.size(); point_idx++)
        {
            if (DistanceToOverhangMapClockwise.find(point_idx) != DistanceToOverhangMapClockwise.end()
                && DistanceToOverhangMapAntiClockwise.find(point_idx) != DistanceToOverhangMapAntiClockwise.end())
            {
                distanceToOverhangAreaMap[point_idx] = DistanceToOverhangMapClockwise[point_idx] > DistanceToOverhangMapAntiClockwise[point_idx] ? DistanceToOverhangMapAntiClockwise[point_idx] : DistanceToOverhangMapClockwise[point_idx];
            }
            else
            {
                distanceToOverhangAreaMap[point_idx] = 1000000;
            }
        }
    };
    std::unordered_map<int, double> distanceToOverhangMap;
    //需要获取悬垂那一层的速度。 简单起见，就用设置的参数计算。15%*speed, 以这个为悬垂区域的速度
    //std::unordered_map<int, std::unordered_map<int, double>> zStepDistanceToOverhangMap;
    ...
    //事实上，这个overhang_mask会包括悬垂区域
    Polygons speedDownMask = overhang_mask;
    //if ((layer_nr == 69 || layer_nr == 68) && speedDownMask.size() > 0)
    //{
    //    SVGHelper::appendPolygon(wall,SVG::Color::BLACK,1,"d:/bridgetest/wall.html");
    //    SVGHelper::appendPolygons(speedDownMask,SVG::Color::RED,1, "d:/bridgetest/wall.html");
    //}

    if (extType == EWallOverhangExtendType::CIRCLE)
    {
        //  ADD @2022-05-16 by CL 修改整圈的速率speed_factor
        do{
            if(non_bridge_config.type == PrintFeatureType::OuterWall || non_bridge_config.type == PrintFeatureType::InnerWall);else{break;}
            if(this->overhang_mask.empty()){break;}

            //if(!overhang_extend_circle){break;}     //  add @2022-06-22 by CL

            coord_t half_line_wigth = non_bridge_config.getLineWidth() / 2;
            if(half_line_wigth <= 10){break;}

            //logCL("$CL$ A type=%d, size=%d, area=%f\n", (int)non_bridge_config.type, overhang_mask.size(), overhang_mask.area());

            Polygons parts; parts.add(wall); parts = parts.difference(parts.offset(-half_line_wigth));
            Polygons masks = this->overhang_mask;
            Polygons parts_masks = parts.intersection(masks);
            bool has_overhang_point = false;

            if(parts_masks.empty()){break;}

            //logCL("$CL$ LayerPlan::addWall parts_masks: size=%d, area=%f \n", parts_masks.size(), parts_masks.area());
            if(!has_overhang_point){
                for(int i=0; i < wall.size(); ++i){
                    const Point& point = wall[i];
                    if(parts_masks.inside(point, true)){
                        has_overhang_point = true;
                        break; //  for(int i=0; i < wall.size(); ++i)
                    }
                }
            }

            parts_masks.simplify();
            //logCL("$CL$ LayerPlan::addWall simplify   : size=%d, area=%f \n", parts_masks.size(), parts_masks.area());
            if(!has_overhang_point){
                for(int j=0; j < parts_masks.size(); ++j){
                    ConstPolygonRef overhang = parts_masks[j];
                    //double area = half_line_wigth * (3000); //std::sqrt(6 * 1000);
                    if(overhang.area() > overhang_area_filter){
                        has_overhang_point = true;
                        break; //  for(int j=0; j < parts_masks.size(); ++j)
                    }
                }

                if(0 && has_overhang_point){ //  only test @2022-07-18 by CL
                    Polygons overhang_mask_a = this->overhang_mask.offset(100);
                    Polygons WallLines;
                    for (int point_idx = 0; point_idx < wall.size(); point_idx++){
                        const Point& p0 = wall[point_idx];
                        const Point& p1 = wall[(point_idx + 1) % wall.size()];
                        Polygon line; line.add(p0); line.add(p1);
                        WallLines.add(line);
                    }

                    Polygons in_Line = overhang_mask_a.intersectionPolyLines(WallLines);
                    logCL("$CL$ in_Line.size() = %d\n", in_Line.size());
                }
            }

            if(!has_overhang_point){break;}

            //  修改整圈的速率speed_factor
            Ratio wall_overhang_speed_factor = mesh.settings.get<Ratio>("wall_overhang_speed_factor");
            //speed_factor = (1.0 - wall_overhang_speed_factor) / 3 + wall_overhang_speed_factor ;
            speed_factor = wall_overhang_speed_factor + 5.0 / non_bridge_config.getSpeed();
            //logCL("$CL$ B speed=%f, overhang=%f, area=%f\n\n", speed_factor, wall_overhang_speed_factor, parts_masks.area());

            //  修改悬垂壁流量
            flow_ratio *= mesh.settings.get<Ratio>("wall_overhang_flow_factor");

        }while(false);
    }
    else if(extType == EWallOverhangExtendType::GRADUALLY_XY)
    {
        //calcDistanceToOverhangMap(overhang_mask, constWall, distanceToOverhangMap);
        //内圈收缩. 外圈内缩效果不好。转折的点太突然了
        //if(overhang_inner_wall_shrink != 0)
        if(non_bridge_config.type == PrintFeatureType::InnerWall && overhang_inner_wall_shrink != 0)
        {
            // 0.4的喷嘴，内壁全部内缩0.03.和内外壁间留出空间。用虹吸解决慢速后溢出的料，解决膨胀问题
            //扩大8个线宽，将内墙也尽量计算在overhangmask中。 相当于向内扩展4个线宽+向外扩展4个线宽。将内墙也尽量覆盖在overhangmask中。这样调整就是内墙在这个局部整体往内缩
            ...
            //如果overhang_inner_wall_shrink=0.4， 有一层直接会少一圈。。需要定位一下
            ...
            //if (overhang_mask.size() > 0)
            //{
            //    SVGHelper::writePolygons(overhang_mask);
            //    SVGHelper::writePolygons(expendOverhangMask);
            //    SVGHelper::writePolygons(diffmask);
            //    SVGHelper::writePolygons(tempWalls);
            //    SVGHelper::writePolygons(shrinkWall);
            //}

            //SVGHelper::writePolygons(expendOverhangMask);

            //修改wall. 悬垂区外的数据不变，悬垂区域内的数据改成收缩后的数据
            //SVGHelper::writePolygon(constWall);
            if (overhang_mask.size() > 0 && shrinkWall.size() > 0)
            {
                //整圈内缩
                wall = shrinkWall[0];
                //同时修改startIdx
                //start_idx = locateFirstSupportedVertex(wall, start_idx);
                if (start_idx > wall.size() - 1)
                {
                    start_idx = wall.size() - 1;
                }
            }
        }
        //else
...
//    }
    ...
    //if (layer_nr == 68 && speedDownMask.size() > 0)
    //{
    //    for (size_t i = 0; i < wall.size(); i++)
    //    {
    //        SVGHelper::appendPoint(wall[i], true, 3, SVG::Color::YELLOW, "d:/bridgetest/wall.html");
    //    }
    //}

    //看前面bridge_wall_mask 和 overhang_mask 的算法。他们算出的面积应该是差不多的
    ...
    //if ((layer_nr == 68) && speedDownMask.size() > 0)
    //{
    //    SVGHelper::appendPolygons(bridge_wall_mask, SVG::Color::GREEN, 1, "d:/bridgetest/wall.html");
    //    SVGHelper::appendPolygon(wall, SVG::Color::BLACK, 1, "d:/bridgetest/wall.html");
    //    SVGHelper::appendPolygons(speedDownMask, SVG::Color::RED, 1, "d:/bridgetest/wall.html");
    //}
    //if (layer_nr == 68 && speedDownMask.size() > 0)
    ...
    //    for (size_t i = 0; i < wall.size(); i++)
    //    {
    //        SVGHelper::appendPoint(wall[i],true,3, SVG::Color::BLUE, "d:/bridgetest/wall.html");
    //    }
    //}
    ...
    //  ADD @2022-06-01 by CL 孔与圆柱 装配体 优化
    do{
        if(non_bridge_config.type == PrintFeatureType::OuterWall)  ;else{break;}
        if(mesh.settings.get<bool>("optimize_wall_hole_printing")) ;else{break;}

        bool isHole = false, inHole = false;

        do{ //  @2022-08-12 by CL
            AkSliceDataStorageRef & contextData = this->ref_data_storage.back();
            {   //  孔壁速度  speed_factor
                isHole = wall_area < 0
                        && MM2_2INT(50) < std::abs(wall_area) && std::abs(wall_area) < MM2_2INT(100)
                        && contextData.hasAnyPolyInTheHole(wall.reverse());
                if(isHole){break;}  //  isHole == true;
                if(wall_area < 0){break;}
            }

            {   //  孔内区域(圆柱)
                Polygon hole;
                inHole = contextData.isThePolygInsideHole(wall, &hole);
                if(inHole){
                    Polygons a_ = tempWalls.offset(non_bridge_config_const.getLineWidth() * 3 / 2);
                    Polygons b_ ; b_.add(hole);
                    Polygons c_ = b_.offset(MM2INT(100)).difference(b_);
                    Polygons z_ = tempWalls.intersection(c_);
                    if(z_.size() <= 0){  //  缝隙较大才调整速度和流量
                        inHole = false;
                    }
                }
            }
        ...
        //if(1 && area < 0) logCL("$CL$ wall %s, %d, isHole = %d, inHole = %d, area = %f, Lines = %d\n", __FUNCTION__, __LINE__, isHole, inHole, area, wall.size());

        if(isHole || inHole){
            speed_factor = std::min(0.2, speed_factor);
            flow_ratio *= 0.92;
        }
    }while(false);

    //  ADD @2022-06-07 by CL jerk速度 （尖塔，微小特征）
    do{

        //  add  @2022-06-08 by CL  微小特征
        if(is_small_feature){} //  ok
        else{
            if(non_bridge_config.type == PrintFeatureType::InnerWall || non_bridge_config.type == PrintFeatureType::OuterWall)  ;else{break;}
            if(mesh.settings.get<bool>("optimize_wall_small_printing"));else{break;}
            if(wall_area > 0 && wall_area < MM2_2INT(8));else{break;}
        }
        //logCL("$CL$ J %s, %d, area = %f, size = %d\n", __FUNCTION__, __LINE__ ,area, this->ptr_path_configs.size());

        if(non_bridge_config.type == PrintFeatureType::InnerWall)
            non_bridge_config.speed_derivatives = this->ptr_mesh_config->inset0_config.speed_derivatives;

        non_bridge_config.akPathFeature.isSmall           = uint32_t(true);

        //double vajk_factor = is_small_feature ? (double)small_feature_speed_factor : (0.5);
        //non_bridge_config.speed_derivatives.acceleration *= vajk_factor;
        //non_bridge_config.speed_derivatives.jerk         *= vajk_factor;
        //non_bridge_config.speed_derivatives.speed = non_bridge_config.speed_derivatives.jerk;
    }while(false);


    // andrew 2022年7月21日16:23:28 识别小面积。
    do {
        if (is_small_feature) { break; } //  ok
        if (non_bridge_config.type == PrintFeatureType::InnerWall || non_bridge_config.type == PrintFeatureType::OuterWall); else { break; }

        //烟囱直径 6.8 。 综合孔7.8mm
        double smallCuboidArea = M_PI * MM2INT(4) * MM2INT(4);
        double polygonLength = wall.polygonLength();
        double likeCircleFactor = std::abs(polygonLength* polygonLength / wall_area);
        if (wall_area > 0 && wall_area < smallCuboidArea)
        {
            non_bridge_config.akPathFeature.isTinyArea = uint32_t(true);
        }
        //图形在正方形到圆之间
        if (likeCircleFactor < 4*4)
        {
            //SVGHelper::writePolygon(wall);
            non_bridge_config.akPathFeature.isLikeCycle = uint32_t(true);
        }
    } while (false);
...
//    };
    ...
    //andrew holeWall反向 20220705 begin
    auto writeWall = [&](int point_idx, Point& p0, const Point& p1)
    {
        ...
        //andrew 速度计算。 如果是渐变，就有个降速buf，否则直接降为前面设定的速度 speed_factor_before
        if (!speedDownMask.empty())
        {
            //这里就兼容了 EWallOverhangExtendType::INSTANT
            ...
            if (distance_to_overhang_start > 0 && distance_to_overhang_start < buffDistance)
            {
                ...
                //以wall_overhang_speed_factor为最低点开始调节速度. 离的越远速度越大。 最大不超过原先设定的速度
                int speedAdjustTime = distance_to_overhang_start / step;

                for (size_t s = 0; s < speedAdjustTime; s++)
                {
                    speed_factor_adjust += speedDownPerMM;
                    if (speed_factor_adjust >= speed_factor_before)
                    {
                        speed_factor_adjust = speed_factor_before;
                        //flow_ratio = flow_ratio_before;
                        break;
                    }
                    else
                    {
                        //降速的时候降流量
                        //flow_ratio = flow_ratio_before* mesh.settings.get<Ratio>("wall_overhang_flow_factor");
                    }
                }
            }
            else if (distance_to_overhang_start > buffDistance)
            {
                speed_factor_adjust = speed_factor_before;
            }
            speed_factor = speed_factor_adjust;
        }
    ...
    //形成闭环，point_idx表示p1, point_idx最终要指向p0才是闭环
    for (unsigned int point_idx = 1; point_idx < wall.size(); point_idx++)
    {
        if (non_bridge_config.type == PrintFeatureType::OuterWall
                ...
                && reverse_hole_outer_wall_print_orient) {
            const Point& p1 = wall[(wall.size() + start_idx - point_idx) % wall.size()];
            writeWall(point_idx, p0, p1);
        }
        else
        {
    ...
    //logCL("$CL$ 2 reverse_hole_outer_wall_print_orient = %d wall.size() = %d\n", reverse_hole_outer_wall_print_orient, wall.size());
    //andrew holeWall反向 20220705 end
...
void ExtruderPlan::forceMinimalLayerTime(double minTime, double minimalSpeed, double travelTime, double extrudeTime)
{
    ...
    if (totalTime < minTime && extrudeTime > 0.0)
    {
        ...
        if (factor < 1.0)
        {
            ...
            setCoolMinLayerTimeSpeedFactor(factor);
        }
        else
        {
            factor = 1.0;
...
void LayerPlan::writeGCode(GCodeExport& gcode)
{
     ...
     *  传入 LayerPlan * this
     *  layerPlan.extruder_plans  每个挤出头的结尾 都应该判断
     **/
    ...
    //andrew 外墙冷却不降速
    bool keepOuterWallSpeed = !mesh_group_settings.get<bool>("cool_min_speed_includes_outer_walls");
    //  TODO: 小区域必须被减速，例如小船烟囱
    ...
    const bool jerk_travel_auto_enabled = true;
    ...
    for(size_t extruder_plan_idx = 0; extruder_plan_idx < extruder_plans.size(); extruder_plan_idx++)
    {
        ...
        for(unsigned int path_idx = 0; path_idx < paths.size(); path_idx++)
        {
            ...
            //  add @2022-06-07 by CL
            GCodePath * path_prev = (path_idx > 1) ? &paths[path_idx-1] : nullptr;
            GCodePath * path_next = (path_idx + 1) < paths.size() ? &paths[path_idx+1] : nullptr;
                ...
                if(jerk_travel_auto_enabled && path.config->isTravelPath()){
                    ...
                    do{ //  add @2022-06-07 by CL
                        ...
                        if(isSmall_prev && isSmall_next){
                            j_new = j_old;
                            break;  //  do not change   //  塔尖 空驶的jerk 自动跟随历史值
                        }
            ...
            //begin andrew 20220615 外墙不受其他最小冷却时间变速，保持外墙打印速度. 解决颜色变化和横纹
...
//            }
            ...
            //不是小微特征的外墙。冷却不降速
            if (keepOuterWallSpeed)
            {
                float ExtruderSpeedFactor = extruder_plan.getExtrudeSpeedFactor();
                if (layer_nr > 160)
                {
                    int x = 0;
                }
                ...
                if((path.config->type != PrintFeatureType::OuterWall
                        || path.config->akPathFeature.isSmall
                        //小圆柱
                        || (path.config->akPathFeature.isTinyArea && path.config->akPathFeature.isLikeCycle)
                    ) && !path.config->isTravelPath())
                {
                    speed *= extruder_plan.getExtrudeSpeedFactor();
                }
            }
            ...
            //end andrew 20220615 外墙不使用最小冷却时间变速，保持外墙打印速度
            ...
            if (path.mesh_id != current_mesh)
            {
                ...
                std::stringstream ss;
                //  只保留 model文件名 @2022-05-12 by CL
                auto found = current_mesh.find_last_of("/\\");found += int(found > 0);
                ss << "MESH:" << current_mesh.substr(found);
