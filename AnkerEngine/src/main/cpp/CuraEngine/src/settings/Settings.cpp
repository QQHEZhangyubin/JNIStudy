...
{
    //Andrew 发版本前临时修改。 将步进填充的填充值修改为步进的起点
    if (key == "infill_line_distance")
    {
        const size_t max_infill_steps = get<size_t>("gradual_infill_steps");
        if (max_infill_steps>0)
        {
            //缩小间距。填充变密. max_infill_steps表示2的N次方
            int value = std::pow(2, max_infill_steps);
            return MM2INT(get<double>(key)/value);
        }
    }
...
const std::string Settings::getAllSettingsString() const
{
    ...
    for (const std::pair<std::string, std::string> pair : settings)
    {
        ...
        //snprintf(buffer, 4096, "\n     -s %s=\"%s\"", pair.first.c_str(), Escaped{pair.second.c_str()}.str);
        sstream << buffer;
    }
    return sstream.str();
}
...
template<> EWallOverhangExtendType Settings::get<EWallOverhangExtendType>(const std::string& key) const
{
    const std::string& value = get<std::string>(key);
    if (value == "gradually_xy")
    {
        return EWallOverhangExtendType::GRADUALLY_XY;
    }
    else if (value == "circle") //It's called 'back' internally because originally this was intended to allow the user to put the seam in the back of the object where it's less visible.
    {
        return EWallOverhangExtendType::CIRCLE;
    }
    else //Default.
    {
