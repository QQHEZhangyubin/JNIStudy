...
class GCodeExportBase : public NoCopy
{
#ifdef BUILD_TESTS
...
protected:
    ...
    virtual void writeFXYZE_CL(const char * G, const Velocity& speed, const int x, const int y, const int z, const double e, const PrintFeatureType& feature) = 0; //  add  @2022-03-02 by CL
...
public:
    ...
    virtual void writeCodeStored(const char* str, int pos, int reserve) = 0; //  add  @2022-03-01 by CL
    ...
    virtual void writeJerkXYZE(char axis, const Velocity &x) = 0;  // add @2022-06-16 by CL XYZE轴Jerk
...
public:  //  override GCodeExportBase
    ...
    // add @2022-06-16 by CL XYZE轴Jerk
    ...
    void writeTravelAcceleration(const Acceleration& acceleration); //TODO
    void writePrintK_M900(double K, double L=-1, int S=-1, int T=0); //TODO

    void finalize(const char* endCode);
public:  //  new function
    const Settings& getSettings_Scene();
    const Settings& getSettings_Extruder();
protected:
    void writeFXYZE_CL(const char *G, const Velocity& speed, const int x, const int y, const int z, const double e, const PrintFeatureType& feature) override;
    void __writeFZ_CL(double z, double zDelta, const PrintFeatureType &feature);

};

class CL_CurrentStatus{
public:

    inline double   getF_block()      {return gcode.currentSpeed;}
    inline double   getX_block()      {return INT2MM(gcode.getPosition().x);}
    inline double   getY_block()      {return INT2MM(gcode.getPosition().y);}
    inline double   getZ_block()      {return INT2MM(gcode.getPosition().z);}
    ...
    inline int    & getL_Nr()         {return akLayerNumber;}       // ak
    inline int    & getL_Count()      {return akLayerCount;}        // ak
    inline int      getE_Nr()         {return gcode.getExtruderNr();}
    inline double   getZ_next()       {return INT2MM(gcode.current_layer_z);}

    inline PrintFeatureType &getType(){return akFreatureType;}
