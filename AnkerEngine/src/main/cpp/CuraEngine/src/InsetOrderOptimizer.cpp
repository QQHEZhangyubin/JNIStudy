...
void InsetOrderOptimizer::processHoleInsets()
{
    ...
    bool outer_inset_first = mesh.settings.get<bool>("outer_inset_first")
    ...
    do{ //  @2022-06-08 by CL 孔壁顺序
        ...
        if(!overhang_hole_mask.empty()){
            outer_inset_first = false;  //  悬垂区域 必须从内向外
        }
        else if(optimize_wall_hole){
            outer_inset_first = true;   //  先打孔壁可以提高精度
        }
    }while(false);
    ...
    for (unsigned int outer_poly_order_idx = 0; outer_poly_order_idx < order_optimizer.polyOrder.size(); ++outer_poly_order_idx)
    {
        ...
        gcode_layer.ptr_path_configs.emplace_back(new GCodePathConfig( mesh_config.inset0_config));
        GCodePathConfig &non_bridge_config = *gcode_layer.ptr_path_configs.back();
        non_bridge_config.akPathFeature.isHoleWall = true;
        ...
        if (hole_inner_walls.size() > 0 && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr)
        {
            ...
            if (outer_inset_first)
            {
                if (extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr)
                {
                    ...
                    gcode_layer.addWalls(hole_outer_wall, mesh, non_bridge_config, mesh_config.bridge_inset0_config, wall_overlapper_0, z_seam_config, wall_0_wipe_dist, flow, retract_before_outer_wall);
                }
        ...
        else if (extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr)
        {
            ...
            if (z_seam_config.type == EZSeamType::SHARPEST_CORNER && inset_polys[0].size() == 2 && PolygonUtils::polygonOutlinesAdjacent(*inset_polys[0][1], *inset_polys[0][0], max_gap * 2))
            {
                ...
                gcode_layer.addWall(hole_outer_wall.back(), point_idx, mesh, non_bridge_config, mesh_config.bridge_inset0_config, wall_overlapper_0, wall_0_wipe_dist, flow, retract_before_outer_wall);
            }
            else
            {
                gcode_layer.addWalls(hole_outer_wall, mesh, non_bridge_config, mesh_config.bridge_inset0_config, wall_overlapper_0, z_seam_config, wall_0_wipe_dist, flow, retract_before_outer_wall);
...
void InsetOrderOptimizer::processOuterWallInsets(const bool include_outer, const bool include_inners)
{
    ...
    do{ //  @2022-05-29 by CL
        ...
        if(optimize_wall_0_order)           ;else{break;}
        if(include_outer)                   ;else{break;}
        if(include_inners)                  ;else{break;}
        if(part_inner_walls.size() > 0)     ;else{break;}

        //  add @2022-06-10 by CL  应该避开悬垂区域
        if(true){
            const Polygons& overhang_mask = gcode_layer.getOverhangMask();
            Polygons port_wall; port_wall.add(outer_wall);
            Polygons overhang_mask_in_part = port_wall.intersection(overhang_mask);
            if(overhang_mask_in_part.empty()) ;else{break;}
        }

        //logCL("$CL$=%s, %d, optimize_wall_0_order = %d\n", __FUNCTION__, __LINE__ ,optimize_wall_0_order);

        if(!(*inset_polys[0][0]).orientation()){
            outer_inset_first = true;
        }

        //  只对孔内的圆柱 做外壁顺序优化  @2022-07-13 by CL
        //bool inHole = gcode_layer.ref_data_storage.back().isThePolygInsideHole(outer_wall);
        //if(!inHole){break;}  // inHole == true;

        {
...

            {
                ...
                if (!outer_inset_first) //  默认从外到里 1234
                {
                    // reverse the optimized order so we end up as near to the outline z-seam as possible
                ...
                int e = orderOptimizer.polyOrder.size(), c = e-1;
                int wall_idx_0 = oo ? 0 : 1;
                int wall_idx_1 = oo ? 1 : e;
                int wall_idx_2 = oo ? 1 : 0;
                int wall_idx_3 = oo ? e : 1;

                //  inner_walls
                for (int i=wall_idx_0; i < wall_idx_1; ++i) {
                    int wall_idx = orderOptimizer.polyOrder[i];
                ...
                //  inner_walls
                for (int i=wall_idx_2; i < wall_idx_3; ++i) {
                    int wall_idx = orderOptimizer.polyOrder[i];
                    if(part_inner_walls.size() > 0 && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr)
                        gcode_layer.addWall(part_inner_walls[wall_idx], orderOptimizer.polyStart[wall_idx], mesh, mesh_config.insetX_config, mesh_config.bridge_insetX_config, wall_overlapper_x, wall_0_wipe_dist, flow_ratio, always_retract);
                }
