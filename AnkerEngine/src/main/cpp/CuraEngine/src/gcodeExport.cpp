...
GCodeExportBase::GCodeExportBase()
    : currentPosition(0,0,MM2INT(23.456))
    , layer_nr(0)
...
void GCodeExportBase::preSetup(const size_t start_extruder)
{
...
void GCodeExportBase::setInitialAndBuildVolumeTemps(const unsigned int start_extruder_nr)
{
    const Scene& scene = Application::getInstance().current_slice->scene;
    const size_t extruder_count = Application::getInstance().current_slice->scene.extruders.size();
    for (size_t extruder_nr = 0; extruder_nr < extruder_count; extruder_nr++)
...
void GCodeExportBase::setInitialTemp(int extruder_nr, double temp)
{
...
const std::string GCodeExportBase::flavorToString(const EGCodeFlavor& flavor) const
{
    switch (flavor)
    {
...
std::string GCodeExportBase::getFileHeader(const std::vector<bool>& extruder_is_used, const Duration* print_time, const std::vector<double>& filament_used, const std::vector<std::string>& mat_ids)
{
    std::ostringstream prefix;
    //  版本，第四位是工艺版本，前三位跟随主程 @2022-07-13 by CL
    //prefix << ";Recompiled by AnkerMake " << "V0.8.3 " << cura::Date::getDate().toStringDashed() << new_line;
    //andrew 2022年11月18日15:33:05 这里去掉版本号。版本号在ankermake中追加到gcode的末尾
    ...
    {
        prefix << ";CompileMode: Executable File" << new_line;
    }
    ...
    case EGCodeFlavor::GRIFFIN:
        ...
        prefix << ";HEADER_VERSION:0.2" << new_line;
        prefix << ";FLAVOR:" << flavorToString(flavor) << new_line;
        prefix << ";GENERATOR.NAME:AnkerSlicer" << new_line;
    ...
    default:
        prefix << ";FLAVOR:" << flavorToString(flavor) << new_line;
        prefix << ";TIME:" << ((print_time)? static_cast<int>(*print_time) : 6666) << "s" << new_line;
        ...
        else if (flavor == EGCodeFlavor::REPRAP || flavor == EGCodeFlavor::MARLIN || flavor == EGCodeFlavor::MARLIN_VOLUMATRIC)
        {
            ...
            std::stringstream usedL, usedW;
            usedL << ";Filament used: ";
            usedW << ";Filament weight: ";
            ...
            if (filament_used.size() > 0)
            {
                for (unsigned i = 0; i < filament_used.size(); ++i)
                {
                    ...
                    {
                        usedL << filament_used[i] / (1000 * extruder_attr[i].filament_area) << "m";
                    }
                    else //Use volumetric filament used.
                    {
                        usedL << filament_used[i] << "mm3";
                    }

                    double material_density_g_per_cm3 = Application::getInstance().current_slice->scene.extruders[i].settings.get<double>("material_density");
                    material_density_g_per_cm3 = material_density_g_per_cm3 < 0.01 ? 1.24 : material_density_g_per_cm3;
                    usedW << filament_used[i] / 1000 * material_density_g_per_cm3 << "g";
                }
            }
            else
            {
            ...
            prefix << usedL.str() << new_line;
            prefix << usedW.str() << new_line;
    ...
    //andrew 2022年11月16日16:23:55 新需求：添加打印最大速度到注释。 因为整个gcode中，速度太多了。这里就获取外墙，填充，内墙，顶底等几个典型速度做比较。 一般就是填充速度是最大速度
    {
...
void GCodeExportBase::setLayerNr(unsigned int layer_nr_) {
    layer_nr = layer_nr_;
}

void GCodeExportBase::setOutputStream(std::ostream* stream)
{
    output_stream = stream;
    *output_stream << std::fixed;
}

bool GCodeExportBase::getExtruderIsUsed(const int extruder_nr) const
{
...
Point GCodeExportBase::getGcodePos(const coord_t x, const coord_t y, const int extruder_train) const
{
...
void GCodeExportBase::setFlavor(EGCodeFlavor flavor)
{
    this->flavor = flavor;
...
EGCodeFlavor GCodeExportBase::getFlavor() const
{
    return flavor;
}

void GCodeExportBase::setZ(int z)
{
    current_layer_z = z;
}

void GCodeExportBase::setFlowRateExtrusionSettings(double max_extrusion_offset, double extrusion_offset_factor)
{
    this->max_extrusion_offset = max_extrusion_offset;
    this->extrusion_offset_factor = extrusion_offset_factor;
}

Point3 GCodeExportBase::getPosition() const
{
    return currentPosition;
}
Point GCodeExportBase::getPositionXY() const
{
    return Point(currentPosition.x, currentPosition.y);
}

int GCodeExportBase::getPositionZ() const
{
    return currentPosition.z;
}

int GCodeExportBase::getExtruderNr() const
{
    return current_extruder;
}

void GCodeExportBase::setFilamentDiameter(const size_t extruder, const coord_t diameter)
{
...
double GCodeExportBase::getCurrentExtrudedVolume() const
{
...
double GCodeExportBase::eToMm(double e)
{
    if (is_volumetric)
    {
...
double GCodeExportBase::mm3ToE(double mm3)
{
    if (is_volumetric)
    {
        return mm3;
...
double GCodeExportBase::mmToE(double mm)
{
    if (is_volumetric)
    {
...
double GCodeExportBase::eToMm3(double e, size_t extruder)
{
    if (is_volumetric)
    {
        return e;
...
double GCodeExportBase::getTotalFilamentUsed(size_t extruder_nr)
{
...
std::vector<Duration> GCodeExportBase::getTotalPrintTimePerFeature()
{
    return total_print_times;
}

double GCodeExportBase::getSumTotalPrintTimes()
{
    double sum = 0.0;
...
void GCodeExportBase::resetTotalPrintTimeAndFilament()
{
...
void GCodeExportBase::updateTotalPrintTime()
{
...
void GCodeExportBase::writeComment(const std::string& unsanitized_comment)
{
...
void GCodeExportBase::writeTimeComment(const Duration time)
{
    //  取消均速的时间估计  delete @2022-09-20 by CL
    //*output_stream << ";TIME_ELAPSED:" << time << new_line;
}

void GCodeExportBase::writeTypeComment(const PrintFeatureType& type)
{
    switch (type)
    {
        ...
        case PrintFeatureType::MoveCombing:
            *output_stream << ";TYPE:MOVE-COMBING" << new_line;
            break;
        case PrintFeatureType::MoveRetraction:
            *output_stream << ";TYPE:MOVE-RETRACTION" << new_line;
            break;
...
void GCodeExportBase::writeLayerComment(const LayerIndex layer_nr)
{
    *output_stream << ";LAYER:" << layer_nr << new_line;
}

void GCodeExportBase::writeLayerCountComment(const size_t layer_count)
{
    *output_stream << ";LAYER_COUNT:" << layer_count << new_line;
}

void GCodeExportBase::writeLine(const char* line)
{
    *output_stream << line << new_line;
}

void GCodeExportBase::writeExtrusionMode(bool set_relative_extrusion_mode)
{
...
void GCodeExportBase::resetExtrusionValue()
{
...
void GCodeExportBase::writeDelay(const Duration& time_amount)
{
    *output_stream << "G4 P" << int(time_amount * 1000) << new_line;
    estimateCalculator.addTime(time_amount);
}

void GCodeExportBase::writeTravel(const Point& p, const Velocity& speed)
{
    writeTravel(Point3(p.X, p.Y, current_layer_z), speed);
}
void GCodeExportBase::writeExtrusion(const Point& p, const Velocity& speed, double extrusion_mm3_per_mm, PrintFeatureType feature, bool update_extrusion_offset)
{
    writeExtrusion(Point3(p.X, p.Y, current_layer_z), speed, extrusion_mm3_per_mm, feature, update_extrusion_offset);
}

void GCodeExportBase::writeTravel(const Point3& p, const Velocity& speed)
{
    if (flavor == EGCodeFlavor::BFB)
    {
...
void GCodeExportBase::writeExtrusion(const Point3& p, const Velocity& speed, double extrusion_mm3_per_mm, PrintFeatureType feature, bool update_extrusion_offset)
{
    if (flavor == EGCodeFlavor::BFB)
    {
...
void GCodeExportBase::writeMoveBFB(const int x, const int y, const int z, const Velocity& speed, double extrusion_mm3_per_mm, PrintFeatureType feature)
{
...
void GCodeExportBase::writeTravel(const coord_t& x, const coord_t& y, const coord_t& z, const Velocity& speed)
{
...
void GCodeExportBase::writeExtrusion(const int x, const int y, const int z, const Velocity& speed, const double extrusion_mm3_per_mm, const PrintFeatureType& feature, const bool update_extrusion_offset)
{
    if (currentPosition.x == x && currentPosition.y == y && currentPosition.z == z)
    {
        return;
...
void GCodeExportBase::writeRetraction(const RetractionConfig& config, bool force, bool extruder_switch)
{
...
void GCodeExportBase::writeZhopStart(const coord_t hop_height, Velocity speed/*= 0*/)
{
    if (hop_height > 0)
    {
        if (speed == 0)
...
void GCodeExportBase::writeZhopEnd(Velocity speed/*= 0*/)
{
    if (is_z_hopped)
    {
        ...
        *output_stream << "G1 F" << PrecisionedDouble{1, speed * 60} << " Z" << MMtoStream{current_layer_z}
                       << "; Z_HopEnd:" << MMtoStream{ is_z_hopped } << new_line;
        assert(speed > 0.0 && "Z hop speed should be positive.");
    }
}

void GCodeExportBase::startExtruder(const size_t new_extruder)
{
...
void GCodeExportBase::switchExtruder(size_t new_extruder, const RetractionConfig& retraction_config_old_extruder, coord_t perform_z_hop /*= 0*/)
{
...
void GCodeExportBase::writeCode(const char* str)
{
    *output_stream << str << new_line;
}

void GCodeExportBase::resetExtruderToPrimed(const size_t extruder, const double initial_retraction)
{
...
void GCodeExportBase::writePrimeTrain(const Velocity& travel_speed)
{
...
void GCodeExportBase::setExtruderFanNumber(int extruder)
{
...
void GCodeExportBase::writeFanCommand(double speed)
{
...
void GCodeExportBase::writeTemperatureCommand(const size_t extruder, const Temperature& temperature, const bool wait)
{
...
void GCodeExportBase::writeBedTemperatureCommand(const Temperature& temperature, const bool wait)
{
    if (flavor == EGCodeFlavor::ULTIGCODE)
...
void GCodeExportBase::writeBuildVolumeTemperatureCommand(const Temperature& temperature, const bool wait)
{
...
void GCodeExportBase::writePrintAcceleration(const Acceleration& acceleration)
{
    switch (getFlavor())
    {
        case EGCodeFlavor::REPETIER:
...
void GCodeExportBase::writeTravelAcceleration(const Acceleration& acceleration)
{
    switch (getFlavor())
    {
        case EGCodeFlavor::REPETIER:
...
void GCodeExportBase::writeJerk(const Velocity& jerk)
{
...
void GCodeExportBase::finalize(const char* endCode)
{
    writeFanCommand(0);
    writeCode(endCode);
...
double GCodeExportBase::getExtrudedVolumeAfterLastWipe(size_t extruder)
{
    return eToMm3(extruder_attr[extruder].last_e_value_after_wipe, extruder);
}

void GCodeExportBase::ResetLastEValueAfterWipe(size_t extruder)
{
    extruder_attr[extruder].last_e_value_after_wipe = 0;
}

void GCodeExportBase::insertWipeScript(const WipeScriptConfig& wipe_config)
{
...
void GCodeExport::writeJerkXYZE(char axis, const Velocity &jerk)
{
    std::string XY_CMN, ZZ_CMN, EE_CMN;
#ifdef ANKER_DEBUG
    {
        auto ToString = [](const std::string& cmn, PrecisionedDouble value) -> std::string{
            std::ostringstream oss;
            oss << cmn << value;
            return oss.str();
        };

        XY_CMN = ToString(" ; OLD_XY_JERK=", PrecisionedDouble{2, clCurrentStatus->getJ_XY_block()});
        ZZ_CMN = ToString(" ; OLD_XY_JERK=", PrecisionedDouble{2, clCurrentStatus->getJ_ZZ_block()});
        EE_CMN = ToString(" ; OLD_XY_JERK=", PrecisionedDouble{2, clCurrentStatus->getJ_EE_block()});
    }
#endif
    switch (axis) {
    case 'X':
    case 'Y':{ // writeJerk(jerk); break;
        if(current_jerk != jerk){
            *output_stream << "M205 X" << PrecisionedDouble{2, jerk} << " Y" << PrecisionedDouble{2, jerk} << XY_CMN << new_line;
            current_jerk = jerk;
            estimateCalculator.setMaxXyJerk(jerk);
        }
        break;
    ...
    case 'Z':{
        if(clCurrentStatus->getJ_ZZ_block() != jerk){
            *output_stream << "M205 Z" << PrecisionedDouble{2, jerk} << ZZ_CMN << new_line;
            clCurrentStatus->getJ_ZZ_block() = jerk;
        }
        break;
    }
    case 'E':{
        if(clCurrentStatus->getJ_EE_block() != jerk){
            *output_stream << "M205 E" << PrecisionedDouble{2, jerk} << EE_CMN << new_line;
            clCurrentStatus->getJ_EE_block() = jerk;
        }
        break;
    }
    default:
