...
bool SnapshotLayerCommand::snapshot_writeLayerComment(const int layer_nr){
    ...
    double upF    = 20.0;     //  增加约束：Z轴抬升速度 20mm/s @2022-10-13 by CL
    int    Z_um   = delayF * ms ;
    ...
    /// 画圆擦嘴 和 回抽   @2023-02-14 by ChunLian
    ...
    ///  直线擦嘴 和 回填   @2023-02-14 by ChunLian
    ...
    ///  恢复位置（5轴）   @2023-02-14 by ChunLian
    ...
    ///  首层拍照抬升 （10，50，100） @2022-09-06 by CL
    ///  Pepper 确认开放 @2022-10-09 by CL
    ///  工艺反馈有打印问题 再次屏蔽 @2022-10-19 by ChunLian
    ///  andrew 2022年10月27日17:30:45 再次开放
    //  拍照抬升 增加参数 控制 @2023-02-14 by ChunLian
    ...
    generalTakePicture1(upAgain);   //  通用的每层 拍照

    if( upAgain ){      //   多次抬升拍照（带延时）

        wipeExtruderCircle();       //  画圆擦嘴 和 回抽

        for(double height : anker_param_ai_height_array){
            char L_Nr_str[16]{0};
            sprintf(L_Nr_str, "%d.%02d", L_Nr, ++coust);      //  L_Nr += 1  不递增
        ...
        wipeExtruderLine();         //  直线擦嘴 和 回填

        recoveryPosition();         //  恢复位置（5轴）
    }
