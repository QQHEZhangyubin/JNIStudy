...
//AABB ConstPolygonRef::aabb(bool forced_recalculation)
//{
//    if(!m_aabb.isValid()  || forced_recalculation){
//        m_aabb.calculate(*this);
//    }
//    return m_aabb;
//}
...
Polygons ConstPolygonRef::unionPolygon(const ConstPolygonRef &other, ClipperLib::PolyFillType fill_type) const
{
    Polygons ret;
    ClipperLib::Clipper clipper(clipper_init);
    clipper.AddPath(*path, ClipperLib::ptSubject, true);
...
Polygons ConstPolygonRef::difference(const ConstPolygonRef &other) const
{
    Polygons ret;
    ClipperLib::Clipper clipper(clipper_init);
    clipper.AddPath(*path, ClipperLib::ptSubject, true);
...
Polygons ConstPolygonRef::xorPolygon(const ConstPolygonRef &other) const
{
    Polygons ret;
    ClipperLib::Clipper clipper(clipper_init);
    clipper.AddPath(*path, ClipperLib::ptSubject, true);
    ...
    if(remove_holes)
    {
        ...
        for (auto it = paths.begin(); it < new_end;)
        {
            ...
            if(fabs(INT2MM2(ClipperLib::Area(*it))) < min_area_size)
            {
                ...
                *it = std::move(*new_end);
                continue;
                //when the first element < min_area_size, it-- throw exception
