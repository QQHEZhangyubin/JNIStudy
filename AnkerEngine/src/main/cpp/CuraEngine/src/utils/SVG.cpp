...
//--------------------andrew-----------
//定义.数据不需要删除了。 
std::unordered_map<std::string, SVG*> SVGHelper::fileMap;
void SVGHelper::writePoint(const Point& p, bool write_coords, int size, SVG::ColorObject color, std::string filename)
{
    SVG svg(filename, AABB(Point(0, 0),  Point(width, heigth)), scale, Point(1024, 1024));
    svg.writePoint(p, write_coords, size, color);
    svg.flush();
}
void SVGHelper::writePolygons(const Polygons& polys, SVG::ColorObject color, float stroke_width , std::string filename)
{
    SVG svg(filename, AABB(Point(0, 0), Point(width, heigth)),scale, Point(1024, 1024));
    svg.writePolygons(polys, color, stroke_width);
    svg.flush();
}
void SVGHelper::writePolygon(ConstPolygonRef poly, SVG::ColorObject color, float stroke_width, std::string filename)
{
    SVG svg(filename, AABB(Point(0, 0), Point(width, heigth)),scale, Point(1024, 1024));
    svg.writePolygon(poly, color, stroke_width);
    svg.flush();
}
void SVGHelper::appendPoint(const Point& p, bool write_coords, int size, SVG::ColorObject color, std::string filename)
{
    if (fileMap.find(filename) == fileMap.end())
    {
        SVG *svg = new SVG(filename, AABB(Point(0, 0), Point(width, heigth)),scale, Point(1024, 1024));
        fileMap.insert(std::pair<std::string, cura::SVG*>(filename, svg));
    }
    fileMap[filename]->writePoint(p, write_coords, size, color);
    fileMap[filename]->flush();
}
void SVGHelper::appendPolygons(const Polygons& polys, SVG::ColorObject color, float stroke_width , std::string filename)
{
    if (fileMap.find(filename) == fileMap.end())
    {
        SVG* svg = new SVG(filename, AABB(Point(0, 0), Point(width, heigth)),scale, Point(1024, 1024));
    ...
    fileMap[filename]->writePolygons(polys, color, stroke_width);
    fileMap[filename]->flush();
}
void SVGHelper::appendPolygon(ConstPolygonRef poly, SVG::ColorObject color, float stroke_width, std::string filename)
{
    if (fileMap.find(filename) == fileMap.end())
    {
        SVG* svg = new SVG(filename, AABB(Point(0, 0), Point(width, heigth)),scale, Point(1024, 1024));
    ...
    fileMap[filename]->writePolygon(poly, color, stroke_width);
    fileMap[filename]->flush();
}
} // namespace cura
