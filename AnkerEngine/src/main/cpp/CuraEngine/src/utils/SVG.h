...
{
public:
    static void writePoint( const Point& p, bool write_coords = false, int size = 5, SVG::ColorObject color = SVG::Color::BLACK, std::string filename = "debug_write.html");
    static void writePolygons(const Polygons& polys, SVG::ColorObject color = SVG::Color::BLACK, float stroke_width = 1, std::string filename = "debug_write.html");
    static void writePolygon(ConstPolygonRef poly, SVG::ColorObject color = SVG::Color::BLACK, float stroke_width = 1, std::string filename = "debug_write.html");
...
private:
    ...
    //以1920*1080显示器24英寸53cm*30cm 计算像素和尺寸的对应关系 1920/530000 = 0.003623
    const static int width = 530000;
    const static int heigth = 300000;
    constexpr static float scale = 0.003623;
};
} // namespace cura
#endif // SVG_H
