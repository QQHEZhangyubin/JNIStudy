...
public:
    ...
    //andrew 2022年7月27日16:48:12 修改路径
    void setPath(ClipperLib::Path& polygon)
    {
        path = &polygon;
    }
    ...
    //AABB aabb(bool forced_recalculation = false);

    //AABB aabb(bool Forced recalculation)
    ...
    //  add @2022-06-23 by CL
    Polygons unionPolygon(const ConstPolygonRef& other, ClipperLib::PolyFillType fill_type = ClipperLib::pftNonZero) const;
    Polygons difference(const ConstPolygonRef& other) const;
    Polygons xorPolygon(const ConstPolygonRef& other) const;
