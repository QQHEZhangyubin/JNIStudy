...
{
    if(std::fabs(factor-1.0) < 0.00001){return;} // add @2022-03-02 by CL
...
bool loadMeshSTL_ascii(Mesh* mesh, const char* filename, const FMatrix4x3& matrix)
{
    ...
    bool isIdentity = (matrix == FMatrix4x3()); //  add @2022-03-02 by CL
    while(fgets_(buffer, sizeof(buffer), f))
    {
        if (sscanf(buffer, " vertex %f %f %f", &vertex.x, &vertex.y, &vertex.z) == 3)
        {
            ...
            {
            case 1:
                v0 = isIdentity ? Point3::fromFloat(vertex.x, vertex.y, vertex.z) : matrix.apply(vertex);
                break;
            case 2:
                v1 = isIdentity ? Point3::fromFloat(vertex.x, vertex.y, vertex.z) : matrix.apply(vertex);
                break;
            case 3:
                v2 = isIdentity ? Point3::fromFloat(vertex.x, vertex.y, vertex.z) : matrix.apply(vertex);
...
bool loadMeshSTL_binary(Mesh* mesh, const char* filename, const FMatrix4x3& matrix)
{
    ...
    bool isIdentity = (matrix == FMatrix4x3());
    Point3 v0, v1, v2;
    for (unsigned int i = 0; i < face_count; i++)
    {
        ...
        if(isIdentity){ //  add  @2022-03-02 by CL
            v0 = Point3::fromFloat(v[0], v[1], v[2]);
            v1 = Point3::fromFloat(v[3], v[4], v[5]);
            v2 = Point3::fromFloat(v[6], v[7], v[8]);
        }
        else{
            v0 = matrix.apply(FPoint3(v[0], v[1], v[2]));
            v1 = matrix.apply(FPoint3(v[3], v[4], v[5]));
            v2 = matrix.apply(FPoint3(v[6], v[7], v[8]));
        }
        mesh->addFace(v0, v1, v2);
    }
    fclose(f);
