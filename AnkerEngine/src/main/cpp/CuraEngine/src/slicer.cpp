...
{
    // OpenMP
#pragma omp parallel for default(none) shared(mesh, zbbox, layers) schedule(dynamic)
...
#pragma omp parallel for default(none) shared(mesh, layers_ref) schedule(dynamic)
    // Use a signed type for the loop counter so MSVC compiles (because it uses OpenMP 2.0, an old version).
...
#pragma omp parallel for default(none) shared(mesh, layers_ref, layer_apply_initial_xy_offset) schedule(dynamic)
    // Use a signed type for the loop counter so MSVC compiles (because it uses OpenMP 2.0, an old version).
    for (int layer_nr = 0; layer_nr < static_cast<int>(layers_ref.size()); layer_nr++)
    {
