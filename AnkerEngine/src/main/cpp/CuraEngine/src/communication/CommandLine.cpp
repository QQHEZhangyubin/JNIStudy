...
void CommandLine::sendPrintTimeMaterialEstimates() const
{
    ...
    {
        sum += Application::getInstance().fffProcessor->getTotalFilamentUsed(extruder_nr);
    }
}
...
{
    Application::getInstance().fffProcessor->time_keeper.restart();
            ...
            else //Starts with "-" but not with "--".
            {
                ...
                switch(argument[1])
                {
                    ...
                    case 'o':
                    {
                        ...
                        argument = arguments[argument_index];
                        if (!Application::getInstance().fffProcessor->setTargetFile(argument.c_str()))
                        {
...
void CommandLine::loadJSONSettings(const rapidjson::Value& element, Settings& settings)
{
    for (rapidjson::Value::ConstMemberIterator setting = element.MemberBegin(); setting != element.MemberEnd(); setting++)
    {
        ...
        //4.11版本的json文件中， leaf节点也有children元素, 不过为空{}
        if (setting_object.HasMember("children") && setting_object["children"].MemberCount() > 0)
        {
