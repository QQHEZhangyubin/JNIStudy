...
{
    Application::getInstance().fffProcessor->setTargetStream(&private_data->gcode_output_stream);
}
...
bool ArcusCommunication::hasSlice() const
{
    ...
    return private_data->socket->getState() != Arcus::SocketState::Closed
        ...
        && private_data->slice_count < 2; //Only slice once per run of CuraEngine. See documentation of slice_count.
}
...
void ArcusCommunication::sendPrintTimeMaterialEstimates() const
{
    ...
    std::vector<Duration> time_estimates = Application::getInstance().fffProcessor->getTotalPrintTimePerFeature();
    ...
    for (size_t extruder_nr = 0; extruder_nr < Application::getInstance().current_slice->scene.extruders.size(); extruder_nr++)
    {
        ...
        material_message->set_material_amount(Application::getInstance().fffProcessor->getTotalFilamentUsed(extruder_nr));
    }
...
void ArcusCommunication::sliceNext()
{
    ...
    logAlways("%4d Received a Slice message. %s\n", __LINE__, __func__);
    ...
    {
        slice.compute();
        Application::getInstance().fffProcessor->finalize();
        flushGCode();
