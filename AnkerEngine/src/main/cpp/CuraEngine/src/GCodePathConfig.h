...
public:
    ...
    struct AkPathFeature{   //  add  @2022-06-08 by CL
        uint32_t isSmall        : 1;
        uint32_t isTop1         : 1;
        uint32_t isBottom1      : 1;
        uint32_t isOuterWall    : 1;
        uint32_t isInnerWall    : 1;
        uint32_t isHoleWall     : 1;
        uint32_t isTinyArea     : 1;
...
private:
    ...
    bool is_bridge_path; //!< whether current config is used when bridging
