...
void WallsComputation::generateInsets(SliceLayerPart* part)
{
    ...
    //  begin 刻度：内凹   add @2022-07-20 by CL    /// 未完善 delete @2022-09-22 by CL
    bool  wall_ruler_concave_convex_enable = false && settings.get<bool>("wall_ruler_concave_convex_enable");    //  刻度优化
    Ratio wall_ruler_concave_percent       = settings.get<Ratio>("wall_ruler_concave_percent");         //  内凹区域
    Ratio wall_ruler_convex_percent        = settings.get<Ratio>("wall_ruler_convex_percent");          //  外凸区域
    Ratio wall_ruler_separate_percent      = settings.get<Ratio>("wall_ruler_separate_percent");        //  内外墙分离率

    Polygons outline_out_gap;   //  从外向内凹
    Polygons outline_in_gap ;   //  从内向外凸
    ...
    for(size_t i = 0; i < inset_count; i++)
    {
        ...
        if (i == 0)
        {
            ...
            if(wall_ruler_concave_convex_enable){
                //  内凹的小区域，扩大 wall_ruler_concave_percent(1/5)的线宽
        ...
        {
            //2022/11/21 Binary 不够一个线宽的细节由外壁去保持，内壁去掉这些细节，确保路径更顺滑，且不影响挤压外壁
            ...
            if(wall_ruler_concave_convex_enable)
            {
                ...
                if( std::abs(wall_ruler_separate_percent) > 0.001 ){
                    ...
                    //  small_out_gap = small_out_gap.offset(line_width_0 * wall_ruler_concave_percent);    //  内凹累加
                    //  small_out_gap = small_out_gap.offset(line_width_0 / 2 + wall_0_inset);              //  外壁路径
                    //  small_out_gap = small_out_gap.offset(+line_width_0 * wall_ruler_separate_percent);  //  侵蚀
